...
public class ExtensionProbe extends TlsServerProbe<ConfigSelector, ServerReport, ExtensionResult> {
    ...
    public List<ExtensionType> getSupportedExtensions() {
        ...
        ProtocolVersion version = configSelector.getScannerConfig().getDtlsDelegate().isDTLS() ? ProtocolVersion.DTLS12
            : ProtocolVersion.TLS12;
        List<ExtensionType> commonExtensions = getCommonExtension(version, suite -> true);
        if (commonExtensions != null) {
            allSupportedExtensions.addAll(commonExtensions);
        }
        commonExtensions = getCommonExtension(version, CipherSuite::isCBC);
        if (commonExtensions != null) {
            allSupportedExtensions.addAll(commonExtensions);
        }
