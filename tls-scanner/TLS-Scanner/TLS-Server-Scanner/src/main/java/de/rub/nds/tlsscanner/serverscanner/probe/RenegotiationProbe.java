...
public class RenegotiationProbe extends TlsServerProbe<ConfigSelector, ServerReport, RenegotiationResult> {
    ...
    private TestResult supportsDtlsCookieExchangeInRenegotiation() {
        ...
        ReceiveTillAction receiveAction = new ReceiveTillAction(new ServerHelloDoneMessage());
        trace.addTlsAction(receiveAction);
        State state = new State(tlsConfig, trace);
        executeState(state);
        if (!receiveAction.isExecuted()) {
    ...
    public boolean canBeExecuted(ServerReport report) {
        if (report.isProbeAlreadyExecuted(TlsProbeType.CIPHER_SUITE)
            ...
            return (report.getCipherSuites() != null
                && (!report.getCipherSuites().isEmpty() || supportsOnlyTls13(report)));
        } else {
            return false;
        }
    }
